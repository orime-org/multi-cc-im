...
> **状态**：v1.5 进行中（2026-05-09）—— **M1 wechat purge 实施中**（DD #86 §11.4），main 上 daemon 处于 transitional state（无 IM adapter，待 M2-M8 lark adapter）。当前 packages：shared / storage-files / term-wezterm / cli-cc / bridge / im-lark TBD（M2）。架构 → [`docs/architecture.md`](docs/architecture.md)；DD → [`docs/superpowers/specs/`](docs/superpowers/specs/)；用户上手 → [`README.md`](README.md) Quick Start；开发命令 → [`docs/dev.md`](docs/dev.md)。Follow-up：M2-M8 lark adapter / Lark intl WSClient 真账号 smoke / 真实环境 smoke / 后续 tg IM adapter / analytics package。
