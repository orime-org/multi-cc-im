...
describe('runSetupHooksCommand', () => {
  ...
  describe('schema (cc upstream compatibility)', () => {
    ...
    it('PreToolUse group 1: matcher="AskUserQuestion", timeout=310 (~5 min for IM-reply hold per DD §10)', async () => {
      // AskUserQuestion holds the hook subprocess polling for an IM-side
      // natural-language reply (DD §6 P1). §10 sub-revision 2026-05-15
      // reverted §9.5's 120s back to 310s after real mobile usage showed
      // 2 min runs out (phone notification + app switch + thumb-typing
      // while out of house exceeds 2 min). Timeout path self-constructs
      // allow + empty answers (no deny channel). Literal string matcher
      // per cc docs (letters/digits/_/| only → literal exact match).
      const r = await run();
      ...
      expect(askGroup!.hooks[0]!.timeout).toBe(310);
    });
