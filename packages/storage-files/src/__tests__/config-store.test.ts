...
describe('ConfigStore', () => {
  ...
  it('returns default config when file does not exist', async () => {
    ...
    expect(cfg.friendly_names).toEqual({});
  ...
  it('persists and reloads full config', async () => {
    ...
    const input = ConfigSchema.parse({
      friendly_names: { 'sid-1': 'web', 'sid-2': 'mobile' },
      acl: { owners: ['me'] },
    ...
    expect(reloaded.friendly_names).toEqual({
      'sid-1': 'web',
      'sid-2': 'mobile',
    });
  ...
  it('writes valid TOML readable by anything', async () => {
    ...
    await store.save(
      ConfigSchema.parse({
        friendly_names: { web: 'web' },
        acl: { owners: ['me'] },
        external_paths: {},
      }),
    );
    const text = await readFile(cfgPath, 'utf8');
    // TOML section names should appear as "[name]" or as inline tables
    // smol-toml stringifies records as inline tables when keys present.
    expect(text).toMatch(/(friendly_names|acl|external_paths)/);
  });
  ...
  it('rejects loading TOML violating zod schema (empty friendly_name)', async () => {
    await writeFile(cfgPath, '[friendly_names]\n"sid-1" = ""\n');
    const store = createConfigStore({ filePath: cfgPath });
    await expect(store.load()).rejects.toThrow();
  });
  ...
  it('rejects saving config that violates schema', async () => {
    const store = createConfigStore({ filePath: cfgPath });
    await expect(
      // bypass TS to construct invalid runtime input
      store.save({
        friendly_names: { 'sid-1': '' },
        acl: { owners: [] },
        external_paths: {},
