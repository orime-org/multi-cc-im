...
type LogLevelName = keyof typeof LEVEL_IDS;

function isLogLevelName(value: string): value is LogLevelName {
  return value in LEVEL_IDS;
}
...
function resolveMinLevel(): number {
  ...
  if (env && isLogLevelName(env)) return LEVEL_IDS[env];
...
function writeLog(level: string, message: string, accountId?: string): void {
  const levelId: number = isLogLevelName(level) ? LEVEL_IDS[level] : LEVEL_IDS.INFO;
  ...
  const entry = JSON.stringify({
    ...
    _meta: {
      ...
      logLevelId: isLogLevelName(level) ? LEVEL_IDS[level] : LEVEL_IDS.INFO,
      logLevelName: level,
    },
    time: toLocalISO(now),
  });
