...
For the full diff, see `git log packages/im-wechat/lib/ilink/`. When syncing, if upstream changes any of these 8 patch points, those changes need to be merged manually so we keep the strict-TS shape, Node-22+ fetch compatibility, and our diagnostic error formatting.
