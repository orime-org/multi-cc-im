...
cc finishes turn → cc fires Stop hook
   ...
   → 三个前置 short-circuit guard（跟 PreToolUse E2-E4 对称）：
       ...
       2. !exists(state/<sid>.IMOrigin) → return void  (cc 这一轮没绑 IM thread)
       3. !isDaemonAlive(stateDir)      → return void  (forward 不可能)
   → 通过则 sweep stale Stop.* + write <sid>.Stop.<ts>
...
chokidar add event → cli-cc adapter.dispatchOne(StopFile)
   → orchestrator.handleStop(p)
       → check IMWork & read <sid>.IMOrigin (B2 ctx)
       → imAdapter.send([prefix] + msg, replyCtx)
       → delete <sid>.IMOrigin                  # ONE-SHOT 语义
   → unlink <sid>.Stop.<ts>
```
...
cc wants to call <tool> → cc fires PreToolUse hook
   ...
   → 四个前置 early-return（按 cost 从低到高排序）：
       ...
       E3. !exists(state/<sid>.IMOrigin)：                        ~0.1ms (stat)
