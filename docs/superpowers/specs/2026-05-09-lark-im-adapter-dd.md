...
- **M5. Interactive card rendering ❌ won't-do v1 (cancelled 2026-05-11)** — Feishu's interactive card button callbacks only deliver via HTTP webhook (not WSClient long-connection), which conflicts with the project's [core constraint #1 "No public IP"](../../../CLAUDE.md). User decision 2026-05-11 (see §11.5): keep the existing `@<tab> /1 /2` text-command flow forever for v1. M5 is **not** scheduled for any future milestone unless Feishu adds WSClient delivery for card callbacks or the project relaxes the no-public-IP constraint.
- **M6. CLI ✅ done** — `multi-cc-im login lark` shipped in PR #89 (M2 alongside `loginLark` + `LarkCredentialsSchema`); refactored in interactive-wizard W7 (PR #102) to route through `AdapterRegistryEntry.persist`, so the non-interactive shortcut shares the wizard's validate + persist path.
- **M7. Daemon orchestration ✅ done** — `apps/multi-cc-im/src/start.ts` re-enabled credentials check (now against `credentials/lark.json` with "run `multi-cc-im login lark` first" hint when missing) + builds `CredentialStore<LarkCredentials>` + creates `imAdapter = createLarkAdapter({ credentialStore, log })` + restored default `createOrchestrator(...)` call passing the three adapters. Further rewired in W5 (PR #100) so the credential check is replaced by the schema-driven adapter selector; `buildLarkAdapter` test seam dropped in favor of `selectAdapter` + registry override.
- **M8. Tests + docs ✅ done** — collapsed into [interactive-start-wizard DD W8](2026-05-10-interactive-start-wizard-dd.md#101-implementation-milestones-post-dd) (PR #103). README EN+CN setup sections rewritten for the W1–W7 single-command flow; transitional banners removed from README / docs/architecture.md / docs/dev.md; `docs/conventions.md` status row + revision log updated. VENDOR.md already absent since wechat purge (M1).

### 11.5 M5 cards open question — resolved 2026-05-11
...
- **Option 1**: cancel M5 entirely, keep the existing `@<tab> /1 /2` text-command flow forever for v1.
- **Option 2**: investigate other Feishu interaction surfaces (reactions / quick replies / message_action) to see if any of them deliver over WSClient.
- **Option 3**: embed a local HTTP listener + require user to set up Cloudflare Tunnel (or similar) for public-webhook ingress — violates [core constraint #1 "No public IP"](../../../CLAUDE.md).

**Final decision** (user 2026-05-11): **Option 1 — cancel M5**.

Rationale:
- The text-command flow has been smoke-tested through W7 + multiple PR cycles (PRs #104–#113) and works reliably. Users type 2 characters in IM instead of tapping a button — a minor UX downgrade compared to native cards, but acceptable.
- Core constraint #1 (no public IP) is a project identity / security-model invariant, not a workload trade-off (CLAUDE.md feedback `feedback_no_workload_optimization` forbids weighing "workload" as a decision dimension; constraint conflicts win).
- Option 3 doesn't just cost more work — it fundamentally violates the project's deployment model. Option 2 was considered but the expected payoff is uncertain (no evidence other surfaces would behave differently) and the same constraint applies to most interactive UI.
