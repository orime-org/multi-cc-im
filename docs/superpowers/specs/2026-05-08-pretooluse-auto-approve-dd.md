...
**Date**: 2026-05-08（**2026-05-09 amendment**：bare `/start` 默认翻成 `auto:true`，原 `/start auto` 已废，新增 `/start off` 切到 ask 模式 — 见下文「§9 默认翻转 amendment」）
**Status**: ⏳ 待用户审 → 锁定 → 实施
...
## 9. 默认翻转 amendment (2026-05-09)

**Topic**: 用户实测后反馈 v1.6 设计的 bare `/start` (auto:false) 默认值跟实际工作流不匹配 — cc 在「分析这个 repo」类一次性命令后会连发几十个 Bash/Edit，每个都得回 `/1` 不现实。同时 PR #71 (IP health-probed dispatcher) + PR #69 (transient retry) 已经把 IM forward 的网络可靠性提升，问题反而变成「user 不需要每次都同意」而不是「user 想确认每次」。

**Change**:

| | 原 (DD §6 锁定) | 新 (2026-05-09 amendment) |
|---|---|---|
| `/start` (无 args) | `auto:false` (ask 模式) | **`auto:true` (auto-approve 模式)** |
| `/start auto` | `auto:true` (auto-approve) | 废弃（现在等价于 bare `/start`） |
| `/start off` | (不识别) | **`auto:false` (opt-in ask 模式)** |
| `/stop` | 删 IMWork | 不变 |

**理由**:
1. **实际场景反馈**: 用户跑 daemon 几天后反馈每次 `/start` 之后还得记得 `/start auto`，几乎都开。bare `/start` 默认 ask 是「保守但 useless」。
2. **网络层 robustness 已上来**: PR #69 + PR #71 让 IM forward 几乎不丢消息；丢的话 hook 也 default-allow 而不是 deny；意味着「ask」跟「auto」的安全性差异比当初想得小。
3. **重要 case 仍有 escape hatch**: 用户在敏感 repo / 想审核每个工具时 `/start off` 一句话切到 ask；`/stop` 完全断 IM forward 任何工具都 cc TUI 走。
4. **daemon start 仍重置 IMWork → OFF**: 翻转默认值不增加 user 不知情下被「always auto」的风险 — daemon 重启 user 必须重发 `/start`，user 主动选择当前 session 的安全态度。

**修改面**: 仅 `packages/bridge/src/router.ts` 的 `case 'start'` 块（args parse 反转 + echo 文案）+ tests + docs。WeixinReplyContext schema / hook decision tree / IMWork file schema / orchestrator imWorkAction handler 一律不动。
