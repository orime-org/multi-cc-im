...
| Permission prompt 转发 IM 审批 | ✓ | **PreToolUse hook + file IPC**：hook 子进程写 `<paneId>_<sid>.PermissionRequest.<id>.json` → daemon forward IM → 用户回 (a) 死语法 `#<tabname> /1`（允许）/`/2`（拒绝） 或 (b) **自然语言**（"multi-cc-im 那个 rm 同意" / "deny the bash one"，v1.7 起；daemon 把当前所有 pending Request 喂同一个 AI router 子进程，AI 决 target+decision+reason 写进 `permissionResponse` 输出字段；非对称信任：allow 必须含 match-signal 否则降级 deny；deny 自由）→ daemon 写 `<paneId>_<sid>.PermissionResponse.<id>.json`（AI 路径用 AI verbatim reason，死语法 fall back 默认串 `"IM user replied /1\|/2"`）→ hook 读完写 stdout `{permissionDecision: "allow"/"deny"}`。10 秒 timeout 默认 allow。AI 路径独立 `[AI permission] target=X decision=Y reason="..."` audit log 行（与 `[AI router]` 区分）。**AskUserQuestion 特判**（v1.9 起）：cc 内置 widget 走 PreToolUse 但**不**走常规 allow/deny 语义。setup-hooks.ts 把 PreToolUse 拆 2 disjoint matchers（AskUserQuestion 走 300s timeout / 其余走 20s）；hook decision tree 加 `isAskUserQuestion` 分支跳过 auto-allow short-circuit（IMWork on 任意模式都 forward）；orchestrator 渲染数字编号选项 + 「你的考虑」自由文本项替代 `/1` `/2`；ai-router prompt PENDING 块多行 bullet 显示 question + options + SPECIAL RULE 节告诉 AI always deny + reason=exact label 或 verbatim；hook 收到 response 强制 `decision='deny'` 把 user answer 走 `permissionDecisionReason` 传给 cc，cc 自己解读 reason 当用户答复，**无 keystroke 注入**；timeout 兜底默认 allow 让 cc TUI 渲染 widget。独立 `[AskUserQuestion forward pane=X tab=Y] questions=N options=M` audit log 行（与 `[AI router]` / `[AI permission]` 区分）。[DD: permission forward](superpowers/specs/2026-05-07-permission-forward-dd.md) + [DD: natural-language permission replies](superpowers/specs/2026-05-11-im-permission-natural-language-dd.md) + [DD: AskUserQuestion IM bridge](superpowers/specs/2026-05-12-askuserquestion-im-bridge-dd.md) |
