...
> **状态**：v1.5 进行中（2026-05-09）—— **M1 wechat purge + M2 lark login + M3 lark adapter 完成**（DD #86 §11.4）。`multi-cc-im login lark` 可用（验证 `app_id` + `app_secret` → 写 `~/.multi-cc-im/credentials/lark.json`）；`createLarkAdapter()` 实现完毕；daemon 仍处 transitional state（M5 interactive cards / M7 orchestrator wiring 进行中）。当前 packages：shared / storage-files / im-lark / term-wezterm / cli-cc / bridge。架构 → [`architecture.md`](architecture.md)；DD → [`superpowers/specs/`](superpowers/specs/)；用户上手 → [`../README.md`](../README.md) Quick Start；开发命令 → [`dev.md`](dev.md)。Follow-up：M5 interactive cards / M7 orchestrator wiring / Lark intl WSClient 真账号 smoke / 真实环境 smoke / 后续 tg IM adapter / analytics package。
